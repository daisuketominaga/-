...
import { sampleProject, emptyProject } from "@/lib/sample";
...
export default function Home() {
  const { project, setProject, replaceProject } = useProject();
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-4">
        ...
        {project && (
          <div className="flex flex-wrap items-center gap-2 text-xs">
            ...
            <button
              ...
              onClick={() =>
                downloadText(
                  `${project.name || "project"}.ichien.json`,
            ...
            <button
              className="btn-ghost"
              onClick={() => {
                if (confirm("サンプル（藤沢市鵠沼松が岡）に戻しますか？今の内容は消えます。"))
            ...
            <button
              className="btn-ghost"
              onClick={() => {
                if (confirm("白紙から始めますか？今の内容は消えます。")) replaceProject(emptyProject());
              }}
            >
              新規
            </button>
      ...
      <nav className="mb-4 flex gap-1 overflow-x-auto rounded-lg bg-white p-1 shadow-sm border border-slate-200">
        {TABS.map((t) => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
      ...
      <footer className="mt-8 text-[11px] leading-relaxed text-slate-400">
        図面は参考図です。面積は概算で、建築には別途設計・建築確認が必要です。境界からの離れは民法234条（50cm）を基本表示し、
        防火地域・準防火地域で外壁が耐火構造の場合（建築基準法65条）は切り替えで非表示にできます。
      </footer>
    </div>
  );
}
